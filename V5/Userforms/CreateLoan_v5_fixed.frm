...
Private currentArticleID As String  ' ← AJOUTÉ: Pour stocker l'ID article
...
' ← AJOUTÉ: Méthode Public pour initialisation depuis LoanHub
...
Private Sub CreateInterface()
    ...
    With lblDateHeure
        .Caption = "date et heure du" & vbCrLf & "prêt"
        .Left = 12
        .Top = 24
        .Width = 90
        .Height = 30
    ...
    With lblDateRetour
        .Caption = "date du retour" & vbCrLf & "prévu"
        .Left = 380
        .Top = 24
        .Width = 80
        .Height = 30
...
Private Sub SearchArticleByQR(qrCode As String)
    ...
    For i = 2 To wsArticles.Cells(wsArticles.Rows.Count, 1).End(xlUp).Row
        If wsArticles.Cells(i, 3).Value = qrCode Then
            txtObjet.Value = wsArticles.Cells(i, 2).Value ' Nom article
            currentArticleID = wsArticles.Cells(i, 1).Value ' ← CORRIGÉ: Stocker ID
            Exit Sub
        End If
    Next i
    
    MsgBox "QR Code non trouvé", vbExclamation
End Sub
...
Private Sub btnValidation_Click()
    ...
    If Not M_Core.ValidateRequiredFields( _
        ...
        txtObjet, "Objet", _
        txtQtePris, "Quantité", _
        txtTechnicien, "Technicien" _
    ) Then Exit Sub
    
    ' Vérifier que l'article a été scanné (ID existant)
    ...
    With wsPrets
        .Cells(newRow, 1).Value = txtNumPret.Value ' ID
        .Cells(newRow, 2).Value = currentTech ' Tech départ
        .Cells(newRow, 3).Value = currentEmprunteur ' Emprunteur
        .Cells(newRow, 4).Value = Now ' Date prêt
        .Cells(newRow, 5).Value = currentArticleID ' ← CORRIGÉ: Article ID (pas nom!)
        .Cells(newRow, 6).Value = txtObjet.Value ' Article nom
        .Cells(newRow, 7).Value = txtQtePris.Value ' Quantité
        .Cells(newRow, 8).Value = txtDateRetour.Value ' Retour prévu
        .Cells(newRow, 9).Value = currentRaison ' Raison
        ' Colonnes 10-15 vides (retour)
    End With
    ...
    MsgBox "Prêt créé avec succès!", vbInformation
