...
Public Sub ValiderRetoursCoches(lstReturns As MSForms.ListBox, checkControls As Collection, technicien As String)
    ...
    For i = 1 To checkControls.Count
        ...
        If chk.Value = True Then
            ' Récupérer ligne Excel depuis Tag de la checkbox
            rowNum = CLng(chk.Tag)
            
            wsPrets.Cells(rowNum, 15).Value = dateRetour
            wsPrets.Cells(rowNum, 13).Value = technicien
...
Public Sub SendLoanEmail(loanData As Object)
    ' Email création prêt
...
Public Function GetDashboardStats() As Object
    ...
    ' ← CORRIGÉ: Type retour As Object (pas Variant)
...
Public Function GetTopArticles(topN As Long) As Object
    ' Retourne dictionnaire des top N articles les plus prêtés
    
    On Error GoTo ErrorHandler
    
    Dim wsPrets As Worksheet
    Dim articlesDict As Object
    Dim i As Long
    Dim articleNom As String
    
    Set wsPrets = ThisWorkbook.Worksheets("prets")
    Set articlesDict = CreateObject("Scripting.Dictionary")
    
    ' Compter prêts par article
    For i = 2 To wsPrets.Cells(wsPrets.Rows.Count, 1).End(xlUp).Row
        articleNom = CStr(wsPrets.Cells(i, 6).Value)
        If articleNom <> "" Then
            If articlesDict.Exists(articleNom) Then
                articlesDict(articleNom) = articlesDict(articleNom) + 1
            Else
                articlesDict.Add articleNom, 1
            End If
        End If
    Next i
    
    Set GetTopArticles = articlesDict
    Exit Function
    
ErrorHandler:
    M_Core.LogError "GetTopArticles", Err.Description
    Set GetTopArticles = Nothing
End Function
